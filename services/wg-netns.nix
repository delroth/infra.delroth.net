...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd = let
      ...
      forwardSockets = builtins.listToAttrs (map (port: {
      ...
      forwardServices = builtins.listToAttrs (map (port: rec {
        name = "wireguard-netns-forward-${toString port}";
        value = {
        ...
        wireguard = {
          ...
          serviceConfig = {
            ...
            ExecStart = pkgs.writers.writeDash "wireguard-up" ''
              ...
              ${pkgs.iproute}/bin/ip -n wireguard addr add ${cfg.ip4}/32 dev wireguard
              ${pkgs.iproute}/bin/ip -n wireguard -6 addr add ${cfg.ip6}/128 dev wireguard
